  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Load method
        const methodRes = await fetch(`/api/admin/shipping/methods/${id}`);
        if (!methodRes.ok) throw new Error("Method not found");
        const method = await methodRes.json();

        setZoneId(method.zoneId);
        setName(method.name);
        setDescription(method.description || "");
        setPrice(method.price.toString());
        setEstimatedDays(method.estimatedDays || "");
        setActive(method.active);
      } catch (err) {
        toast.error("Failed to load method");
